"""
Clinical Algorithmic Engine & Guideline Rules for OncoCardio Sentinel: Anthracycline & Immune Checkpoint Myocarditis Risk Agent.
...
class ClinicalDomainEngine:
    GUIDELINE = "ESC 2022 Cardio-Oncology Guidelines"
    ...
    def evaluate_primary_index(cls, value: float) -> Optional[Dict[str, Any]]:
        if value > cls.PRIMARY_BASELINE_LIMIT:
            return {
                ...
                "finding": f"Observed value ({value:.2f}) exceeds ESC 2022 Cardio-Oncology Guidelines clinical baseline limit ({cls.PRIMARY_BASELINE_LIMIT:.1f}).",
    ...
    def evaluate_biomarker_concordance(cls, status_flag: str, biomarkers: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        if "DISCORDANT" in status_upper or "EQUIVOCAL" in status_upper or "MUTANT" in status_upper:
            return {
                ...
                "recommendation": f"Order reflex confirmatory testing per ESC 2022 Cardio-Oncology Guidelines clinical recommendations.",
            }
        return None
